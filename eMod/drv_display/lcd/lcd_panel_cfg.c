...
static void LCD_cfg_panel_info(__panel_para_t * info)
{
	...
	info->lcd_x                   = 720;
    info->lcd_y                   = 720;
    info->lcd_dclk_freq           = 29;  //33MHz    // 不能低于22MHz
...
static __s32 LCD_open_flow(__u32 sel)
{
	...
	LCD_OPEN_FUNC(sel, LCD_io_init	, 100); 	//申请和初始化IO，并延时20ms
	LCD_OPEN_FUNC(sel, TCON_open	, 50); //打开LCD控制器，并延时200ms
	LCD_OPEN_FUNC(sel, LCD_open_cmd	, 20); 	//通过IO配置LCD模组为工作模式，并延时10ms
...
{	
	...
	LCD_CLOSE_FUNC(sel, LCD_power_off   , 1000); //关闭LCD供电，并延时1700ms
