...
__bool RDA5807P_ValidStop(__u16 freq)
{
	...
	RDA5807P_reg_data[3]=(((curChan&0x0003)<<6)|0x10) | (RDA5807PE_initialization_reg[3]&0x0f);
	...
	if (falseStation==1)
	{
		__msg("auto search failed!\n");
		return 0;   
	}
	else 
	{		
		__msg("auto search successed!\n");
		//__wait__;
		return 1;
	}
}
...
#else
	RDA5807PE_initialization_reg[7]=( RDA5807PE_initialization_reg[7] & 0xf0 ) | level; 
	OperationRDAFM_2w(WRITE, &(RDA5807PE_initialization_reg[0]), 8);
#endif
}
