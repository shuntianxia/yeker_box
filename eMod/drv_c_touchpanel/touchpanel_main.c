...
****************************************************************************************************
*/
static void  touchpanel_data_task(void *p_arg)
{
    __u8 err;
    __s32 pin;
    ...
    while(1)
    {
        ...
        esKRNL_SemPend(tp_private.tp_sem_data, 0, &err);
        ctp_catch_data_normal();
        esKRNL_SemPost(tp_private.tp_sem_main);
    }
}

static __bool touchpanel_main_task(void *arg)
{
    __u8 err;
    __s32 pin;
    ...
    while(1)
    {
        ...
        pin=esPINS_ReadPinData(tp_private.h_gpio_irq, 0);
        if (pin == 0)
        {
            delay_nus(50);
            pin=esPINS_ReadPinData(tp_private.h_gpio_irq, 0);
...
__s32 touchpanel_init(void)
{
    ...
    tp_private.main_tid = esKRNL_TCreate(touchpanel_main_task, 0, 0x800, KRNL_priolevel6);
    tp_private.data_tid = esKRNL_TCreate(touchpanel_data_task, 0, 0x800, KRNL_priolevel3);
...
__s32 touchpanel_exit(void)
{
    ...
    if(tp_private.data_tid)
    {
        esKRNL_SemPost(tp_private.tp_sem_data);
        esKRNL_TDel(tp_private.data_tid);
        esKRNL_TimeDly(4);
    }
