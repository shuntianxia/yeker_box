...
typedef struct {
    ...
    int cursor;
    int points;
    MwMotionDirect x_y; // Direction of X or Y
    int attr;
...
static inline int iner_dclick_ontimer(icb_t * icb, __gui_msg_t * msg);
...
static inline int iner_sendmessage(icb_t * icb, __gui_msgid_user_t type,
                                        int x, int y, MwMotionDirect dir, int to)
{
    __gui_msg_t     msgex;
    int ret;
    msgex.id        = type;                         // Gesture type
    msgex.h_srcwin  = NULL;
    msgex.h_deswin  = icb->cls.msg_win;
...
#ifdef THIS_DEBUG
{
    ...
    log_mi("%x, to=%d, ret = %d", type, to, ret);
    if (ORANGE_OK == GUI_WinGetName(msgex.h_deswin, name)) {
        log_mi("Destination window name=%s", name);
    }
}
#endif
    return ret;
...
static inline int iner_sendmessage_todesktop(icb_t * icb, __gui_msgid_user_t type,
                                        int x, int y, MwMotionDirect dir, int to)
{
    __gui_msg_t     msgex;
    int ret;
...
int iner_close(void * hdl)
{
    ...
    GUI_KillTimer(icb->cls.h_win, icb->cls.timerid);
    esMEMS_Mfree(0, icb);
    return 0;
}
...
void iner_ontimer(void * hdl, __gui_msg_t * msg)
{
    ...
    if (TMID_DOUBLE_CLICK == icb->mo.timer_id) {
        iner_dclick_ontimer(icb, msg);
    } else if (TMID_INERTIA_SBLOCK == icb->mo.timer_id) {
        iner_sblock_ontimer(icb, msg);
    } else if (TMID_INERTIA_BLOCKS == icb->mo.timer_id) {
        iner_blocks_ontimer(icb, msg);
    } else {
        log_e("Error timer id");
    }
}
...
int iner_ondown(void * hdl, __gui_msg_t * msg, int x, int y, iner_start_t * cls)
{
    ...
    if (PSTAT_READY == icb->mo.progress) {
        if (0 == cls->attr_x && 0 == cls->attr_y) {
            log_w("No gesture attribute");
            return -1;
        }

        icb->mo.progress = PSTAT_CLICK;

        eLIBs_memcpy(&icb->cls, cls, sizeof(iner_start_t));
        icb->mo.sample_x[0] = x;
        icb->mo.sample_y[0] = y;
        icb->mo.cursor = 0;
        icb->mo.points = 1;
        GUI_KillTimer(icb->cls.h_win, icb->cls.timerid);
        if ((icb->cls.attr_x & MW_GESTURE_ATTR_DCLICK) && icb->mo.dclick) {
            icb->mo.dclick = 0;
            iner_sendmessage(icb, GUI_MSG_USER_GESTURE_DCLICK, x, y, MW_MOTDIR_NONE, 0);
            icb->mo.progress = PSTAT_READY;
            return 0;
        }
        if (icb->cls.attr_x & MW_GESTURE_ATTR_LPRESS) {
            icb->mo.click_tm = GUI_GetTickCount();
        }
    } else {
        log_e("Gesture state machine initiated state error, %d", icb->mo.progress);
        return -1;
    }
    return 0;
}
...
int iner_onmove(void * hdl, __gui_msg_t * msg, int x, int y)
{
    ...
    if (PSTAT_CLICK == icb->mo.progress) {
        if (x != icb->mo.sample_x[0] || y != icb->mo.sample_y[0]) {
            icb->mo.progress = PSTAT_MOVE;

            if (0 == (icb->cls.attr_x & MW_GESTURE_ATTR_COMB_MOVE)) {
                icb->mo.x_y         = MW_MOTDIR_TOP_DOWN;
            ...
            } else if (0 == (icb->cls.attr_y & MW_GESTURE_ATTR_COMB_MOVE)) {
                icb->mo.x_y         = MW_MOTDIR_LEFT_RIGHT;
            ...
            } else if (MW_MATH_ABS(x - icb->mo.sample_x[0]) >= (MW_MATH_ABS(y - icb->mo.sample_y[0]) * 3)) {
            ...
            if (0 == (icb->mo.attr & MW_GESTURE_ATTR_COMB_MOVE)) {
                icb->mo.progress = PSTAT_READY;
                return 0;
            }
            iner_sendmessage_todesktop(icb, GUI_MSG_USER_GESTURE_NEIGHBOR, x, y, icb->mo.x_y, icb->mo.displace);
#ifdef THIS_DEBUG
            log_mi("the direction is %d", icb->mo.x_y);
#endif
       }else if (icb->cls.attr_x & MW_GESTURE_ATTR_LPRESS) {
            tm = GUI_GetTickCount();
            if ((tm - icb->mo.click_tm) > MW_LPRESS_PERIOD) {
                iner_sendmessage(icb, GUI_MSG_USER_GESTURE_LPRESS, x, y, MW_MOTDIR_NONE, 0);
                icb->mo.progress = PSTAT_READY;
                return 0;
            }
        }
    ...
    if (PSTAT_MOVE == icb->mo.progress) {
        ...
        if (MW_MOTDIR_LEFT_RIGHT == icb->mo.x_y) {
            to = x;
        } else {
            to = y;
        }
        if (icb->mo.attr & MW_GESTURE_ATTR_POSITIVE) {
            icb->mo.displace += (to - icb->mo.latest);
        } else {
            icb->mo.displace -= (to - icb->mo.latest);
        }
        icb->mo.latest = to;
        iner_sendmessage(icb, GUI_MSG_USER_GESTURE_MOVE, x, y, icb->mo.x_y, icb->mo.displace);
        MW_MATH_CYCLE_INC_0(icb->mo.cursor, MW_SAMPLE_POINTS);
        icb->mo.sample_x[icb->mo.cursor] = x;
        icb->mo.sample_y[icb->mo.cursor] = y;
        icb->mo.points++;
#ifdef THIS_DEBUG
        log_mi("Move displace: %d", icb->mo.displace);
#endif
    } else {
#ifdef THIS_DEBUG
        log_e("Wrong move state machine, %d", icb->mo.progress);
#endif
        return -1;
    }
    return 0;
...
int iner_onup(void * hdl, __gui_msg_t * msg, int x, int y)
{
    ...
    if (PSTAT_CLICK == icb->mo.progress) {
        ...
        if (icb->cls.attr_x & MW_GESTURE_ATTR_DCLICK) {
            icb->mo.dclick = 1;
            icb->mo.timer_id = TMID_DOUBLE_CLICK;
            GUI_SetTimer(icb->cls.h_win, icb->cls.timerid, MW_DCLICK_INTERVAL, NULL);
        } else if (icb->cls.attr_x & MW_GESTURE_ATTR_CLICK) {
            iner_sendmessage(icb, GUI_MSG_USER_GESTURE_CLICK, x, y, MW_MOTDIR_NONE, 0);
        }
        icb->mo.progress = PSTAT_READY;
    } else if (PSTAT_MOVE == icb->mo.progress) {
        if (icb->mo.attr & MW_GESTURE_ATTR_SLIDE) {
            // No inertial motion attribute
            iner_sendmessage(icb, GUI_MSG_USER_GESTURE_DONE, x, y, icb->mo.x_y, icb->mo.displace);
            icb->mo.progress = PSTAT_READY;
            return 0;
        }
        ...
        if (icb->mo.points < MW_SAMPLE_POINTS) {
            ...
            pre = icb->mo.cursor;
            MW_MATH_CYCLE_INC_0(pre, MW_SAMPLE_POINTS);
        }
    ...
    {
        int i;
        log_mi("Cur pointer %d of %d, pre %d", icb->mo.cursor, icb->mo.points, pre);
        for (i = 0; i < MW_SAMPLE_POINTS; i++) {
            log_mi("%d", icb->mo.sample_x[i]);
        }
        log_mi("Delta: %d, vi %d", icb->mo.displace, icb->mo.vi);
    }
#endif
        icb->mo.sample_x[0] = x;
        icb->mo.sample_y[0] = y;

        if (icb->mo.attr & MW_GESTURE_ATTR_INER) {
    //        iner_motion_free(icb, msg);
        } else if (icb->mo.attr & MW_GESTURE_ATTR_INER_SBLOCK) {
            iner_sblock_launch(icb, msg);
        } else if (icb->mo.attr & MW_GESTURE_ATTR_INER_BLOCKS) {
...
static inline int iner_dclick_ontimer(icb_t * icb, __gui_msg_t * msg)
{
    GUI_KillTimer(icb->cls.h_win, icb->cls.timerid);
    icb->mo.dclick = 0;
    if (icb->cls.attr_x & MW_GESTURE_ATTR_CLICK) {
        iner_sendmessage(icb, GUI_MSG_USER_GESTURE_CLICK, icb->mo.sample_x[0], icb->mo.sample_y[0], MW_MOTDIR_NONE, 0);
    }
    log_mi("Kill dclick timer");
    return 0;
}
...
static inline void iner_sblock_init(icb_t * icb)
{
    ...
    // log_mi("Iner initial velocity and distance: %d, %d", icb->mo.vi, icb->mo.s);
...
static inline int iner_sblock_launch(icb_t * icb, __gui_msg_t * msg)
{
...
#ifdef THIS_DEBUG
    log_mi("Start single block scrolling: block No.=%d, init_block=%d, vi=%d", tmp, icb->mo.init_block, icb->mo.vi);
#endif
    if (icb->mo.init_block == tmp) {
        ...
        if (icb->cls.tailboundary) {
            ...
            if (icb->mo.vi > MW_CRITICAL_VELOCITY) {
                ...
                icb->mo.vi = MW_MATH_ABS(icb->mo.vi);
                if (icb->mo.vi > MW_CRITICAL_VELOCITY) {
                    icb->mo.direction = 1;
        ...
        GUI_SetTimer(icb->cls.h_win, icb->cls.timerid, MW_TIMER_COEFFICIENT, NULL);
#ifdef THIS_DEBUG
        log_mi("Free scroll: dire = %d, location = %d, vc = %d, s = %d",
    ...
    // Display 1 and 2 windows direction
        ...
        log_e("Wrong displacement %d", icb->mo.displace);
        icb->mo.displace = block_edge(icb->mo.init_block, icb->mo.steps);
        iner_sendmessage(icb, GUI_MSG_USER_GESTURE_ERROR,
...
static inline int iner_sblock_ontimer(icb_t * icb, __gui_msg_t * msg)
{
    ...
    if (PSTAT_INERTIA_MOTION != icb->mo.progress) {
        GUI_KillTimer(icb->cls.h_win, icb->cls.timerid);
        iner_sendmessage(icb, GUI_MSG_USER_GESTURE_ERROR, 0, 0, MW_MOTDIR_NONE, 0);
        icb->mo.progress = PSTAT_READY;
        log_e("Wrong handle about ontimer");
        return -1;
    }
    for (i = icb->mo.vi; i > 0; i--) {
    ...
    // log_mi("Timer : loc = %d, vi = %d, run = %d", icb->mo.displace, icb->mo.vi, icb->mo.run);
    if (icb->mo.run >= icb->mo.s) {
        GUI_KillTimer(icb->cls.h_win, icb->cls.timerid);
        iner_sendmessage(icb, GUI_MSG_USER_GESTURE_DONE, 0, 0, icb->mo.x_y, icb->mo.displace + (icb->mo.s * icb->mo.direction));
        icb->mo.progress = PSTAT_READY;
    } else {
        iner_sendmessage(icb, GUI_MSG_USER_GESTURE_MOVE, 0, 0, icb->mo.x_y, icb->mo.displace + (icb->mo.run * icb->mo.direction));
    }
    return 0;
}
...
static inline int iner_blocks_launch(icb_t * icb, __gui_msg_t * msg)
{
    ...
    if (0 == (icb->mo.attr & MW_GESTURE_ATTR_POSITIVE)) {
        icb->mo.direction = -icb->mo.direction;
    }
    ...
    GUI_SetTimer(icb->cls.h_win, icb->cls.timerid, MW_TIMER_COEFFICIENT, NULL);
#if 1
    log_mi("Free scroll: dire = %d, displace = %d, vi = %d",
...
static inline int iner_blocks_ontimer(icb_t * icb, __gui_msg_t * msg)
{
    ...
    log_mi("Timer : loc = %d, vi = %d", icb->mo.displace, icb->mo.vi);
    if (icb->mo.vi < MW_MIN_VELOCITY && (0 == icb->mo.displace % icb->mo.steps)) {
        GUI_KillTimer(icb->cls.h_win, icb->cls.timerid);
        iner_sendmessage(icb, GUI_MSG_USER_GESTURE_DONE, 0, 0, icb->mo.x_y, icb->mo.displace);
        icb->mo.progress = PSTAT_READY;
    } else {
        iner_sendmessage(icb, GUI_MSG_USER_GESTURE_MOVE, 0, 0, icb->mo.x_y, icb->mo.displace);
    }
    return 0;
}
