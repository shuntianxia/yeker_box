...
#define MW_SYS_FONT_PATH2		"d:\\res\\fonts\\font16.sft"
...
static __s32 scrollingPageProc(__gui_msg_t * msg)
{
    ...
    case GUI_MSG_CREATE:
	    ...
	    if(i > 0){
	        wndDpt->background = user_theme_open(i);
			wndDpt->backgroundData = user_theme_hdl2buf(wndDpt->background);
		}else{
        ...
        break;
    case GUI_MSG_DESTROY:
        user_theme_close(wndDpt->background);
        esMEMS_Mfree(0, wndDpt);
        break;
    case GUI_MSG_PAINT:
	...
	case GUI_MSG_USER_GESTURE_CLICK:
	   ...
	   if(SLIB_strcmp(wndDpt->xml_button_controltype,"textview") == 0){//文本框
			...
			if(wndDpt->buttonStatus_close == 0){
			 ...
			 Buttonhandle_Search_Up(wndDpt->code_handle);//点击发码按键是弹起状态 up
