...
static __s32 TimeTopWndProc_1(__gui_msg_t * msg)
{
	...
	case GUI_MSG_CREATE:
		...
		wndDpt->on = dsk_theme_open(ID_CLOCK_UP_O_BMP);
		wndDpt->ondata = dsk_theme_hdl2buf(wndDpt->on);
		wndDpt->off = dsk_theme_open(ID_CLOCK_UP_C_BMP);
	...
	case GUI_MSG_PAINT:
		...
		wndDpt->num = dsk_theme_open(data[hour/10]);
		wndDpt->numdata = dsk_theme_hdl2buf(wndDpt->num);
...
static __s32 TimeTopWndProc_2(__gui_msg_t * msg)
{
	...
	case GUI_MSG_PAINT:
		...
		wndDpt->num = dsk_theme_open(data[min/10]);
		wndDpt->numdata = dsk_theme_hdl2buf(wndDpt->num);
...
static __s32 TimeTopWndProc_3(__gui_msg_t * msg)
{
	...
	case GUI_MSG_CREATE:
		...
		wndDpt->ewii.gestureArgs.attr_x|= GG_GESTURE_ATTR_CLICK;
		wndDpt->ewii.gestureArgs.hmsg_x[GG_GESTURE_MSG_DEST_CLICK] = msg->h_deswin;
		wndDpt->on = dsk_theme_open(ID_CLOCK_DOWN_O_BMP);
		wndDpt->ondata = dsk_theme_hdl2buf(wndDpt->on);
		wndDpt->off = dsk_theme_open(ID_CLOCK_DOWN_C_BMP);
		wndDpt->offdata= dsk_theme_hdl2buf(wndDpt->off);
		esTIME_GetTime(&time);
		hour = time.hour;
		GUI_SetBkColor(0);
...
static __s32 TimeBottomWndProc(__gui_msg_t * msg)
{
	...
	case GUI_MSG_CREATE:
		...
		wndDpt->ewii.hWnd = msg->h_deswin;
		wndDpt->bg = dsk_theme_open(ID_CLOCK_SETTIME_BG_BMP);
