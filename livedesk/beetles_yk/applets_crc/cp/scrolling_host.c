       ...
       //模板保护会出错
		...
		if(wndDpt->geatrue_direction == 1){
			if(wndDpt->clock_en == 1){
				if(page_now != (LOSWORD(msg->dwAddData2) / 720 - 1)){
					XML_haudongtiao_Get_PageHandle(LOSWORD(msg->dwAddData2) / 720 - 1);
					page_now = (LOSWORD(msg->dwAddData2) / 720 - 1);
				}
			}else{
				if(page_now != (LOSWORD(msg->dwAddData2) / 720)){
					XML_haudongtiao_Get_PageHandle(LOSWORD(msg->dwAddData2) / 720);
					page_now = (LOSWORD(msg->dwAddData2) / 720);
				}
			}
		}else if(wndDpt->geatrue_direction == 2){
			if(wndDpt->clock_en == 1){
