...
{		
	...
	/*{
        ...
        __inf("Touch panel open successful");
...
static __msrv_msg_t  *get_msg_buf(__memit_ctr_t *emit)
{
	...
	emit->sys_msg_counter = (emit->sys_msg_counter + 1) % SYS_MSGQ_SIZE;
...
#if 0
	__inf("--------------------Tp msg------------------------\n");	
	__inf("pmsg->id = %d, speed = %d\n", pmsg->id, LOSWORD(pmsg->reserved));
...
{	
	...
	{
		pEventFrame = &(pPacket->events[i]);
		__inf("Key Event %d: type = %d, code = %d, value = %d\n", i + 1,
                                                                 pEventFrame->type,
                                                                 pEventFrame->code,
                                                                 pEventFrame->value
                                                                 );
...
__s32 msg_srv_dispatch_message(__memit_ctr_t *emit, __msrv_msg_t *p_msg)
{
	...
	if((p_msg->type == GUI_MSG_TOUCH) || (p_msg->type == GUI_MSG_KEY))
	{			
		msg.id 			= p_msg->type;
		msg.h_srcwin 	= NULL;		
		msg.h_deswin    = NULL;//GUI_GetActiveManWin();
		msg.dwAddData1	= p_msg->id;
		msg.dwAddData2	= p_msg->data;
		msg.dwReserved	= p_msg->reserved;
		msg.p_arg		= NULL;
				
		//__msg("x = %d, y = %d\n", LOSWORD(p_msg->data), HISWORD(p_msg->data));	
		//__msg("msg_hook: %x\n", emit->msg_hook);
		
		esKRNL_SemPend(emit->p_hook_sem, 0, &err);			
		if(emit->msg_hook)
		{			
			esKRNL_CallBack(emit->msg_hook, &msg);
		}
...
__s32 msg_emit_register_hook(__memit_hook_t *hook)
{
	...
	emit_ctr.msg_hook = esKRNL_GetCallBack((__pCBK_t)hook->msg_hook);
	
	__msg("==================%s==============msg_hook: %x, emit_ctr.msg_hook: %x\n", __func__, hook->msg_hook, emit_ctr.msg_hook);
