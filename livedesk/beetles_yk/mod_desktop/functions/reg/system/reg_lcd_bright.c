...
#define REGNODE_LCD_BRIGHT "\\system\\lcd_bright"
...
__lion_bright_t lion_reg_lcd_bright_get(void)
{
	...
	h_node = eLIBs_RegOpenNode(REGNODE_LCD_BRIGHT, ELIBS_REG_READWRITE);
	if(!h_node)
	{
    ...
    eLIBs_RegGetKeyValue(h_node, "lcd_bright", "current", key_value);
    ...
    else if(!eLIBs_strcmp(key_value, "level16"))
    {
		ret = LION_BRIGHT_LEVEL16;
    }
    else
    {
...
__s32 lion_reg_lcd_bright_set(__lion_bright_t bright)
{
	...
	else if(bright == LION_BRIGHT_LEVEL16)
    {
        eLIBs_strcpy(key_value, "level16");
    }
    else
    {
        eLIBs_strcpy(key_value, "level11");
    ...
    eLIBs_RegSetKeyValue(h_node, "lcd_bright", "current", key_value);
...
__lion_bright_t lion_reg_lcd_bright_get_default(void)
{
    ...
    eLIBs_RegGetKeyValue(h_node, "lcd_bright", "default", key_value);
