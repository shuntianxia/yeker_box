...
__s32 dsk_keytone_init( const char *keytone_file )
{
	...
	keytone->tonebuf 	= (__u8 *)esMEMS_Palloc((SOCKET_SIZE >> 10), MEMS_PALLOC_MODE_BND_NONE | MEMS_PALLOC_MODE_BNK_NONE);
	if( !keytone->tonebuf )
	{
...
__s32 dsk_keytone_exit(void)
{
	if(keytone)
	{
    	...
    	esMEMS_Pfree((void *)keytone->tonebuf, (SOCKET_SIZE >> 10));
    	g_free(keytone);
    	keytone = NULL;
