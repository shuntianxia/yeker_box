...
*************************************************************************************/
__s32 dsk_Systheme_init( const char *file )
{
	if( sys_hres == NULL )
	{
		sys_hres = OpenRes( (char *)file, 0);
		if( sys_hres == NULL )
		{
			__err("Error in opening resource file \"%s\".\n", file);
			return EPDK_FAIL;
...
*****************************************************************************/
HTHEME dsk_Systheme_open( __u32 theme_id )
{
	__s32	  ret;
	HTHEME_i  htheme = NULL;
	void*	  pbuf;
	...
	htheme->id = theme_id;
	htheme->size_com = GetResSize(sys_hres, style, theme_id );
	pbuf = esMEMS_Balloc(htheme->size_com);
	if (NULL == pbuf)
	{
		__msg("*************mem not enough***********\n");
	...
	ret = GetRes(sys_hres, 0, theme_id, pbuf, htheme->size_com);
/*	
	__msg("hres=%x,theme_id=%d,pbuf=%x,htheme->size_com=%d\n",
			hres,theme_id, pbuf, htheme->size_com);*/
	if (-1 == ret)
...
//        __wrn("p[0]=0x%x, p[1]=0x%x", p[0], p[1]);
    }
    return htheme->buf;
}
...
void *dsk_Systheme_hdl2buf( HTHEME handle )
{
	__s32	  size;
	__s32	  ret;
	void*	  pbuf;
	...
	ret = GetRes( sys_hres, style, htheme->id, pbuf, htheme->size_com );
	if (-1 == ret)
	{
		__msg("***********GetRes fail***********\n");
		esMEMS_Bfree(htheme->buf, htheme->size);
